...
function SaathiApp({ user, onSignOut }) {
  ...
  const sendMessage = async (text) => {
    ...
    const memoryInstruction =
      lang === "hi"
        ? `अगर इस बातचीत में यूज़र ने कोई नई महत्वपूर्ण जानकारी बताई (नाम, पसंद-नापसंद, बड़ी बात, रिश्ता, जन्मदिन, कोई समस्या), तो अपने जवाब के बिल्कुल आखिर में एक नई लाइन में लिखो: [MEMORY: fact1 | fact2] — यह लाइन यूज़र को नहीं दिखेगी, सिर्फ याद रखने के लिए है। अगर कुछ नया नहीं है तो यह लाइन बिल्कुल मत लिखो।`
        : `If the user shared any new important info this turn (name, preferences, a big event, relationship, birthday, a problem), end your reply on a new line with: [MEMORY: fact1 | fact2] — this line is hidden from the user, only for remembering. If nothing new, don't include this line at all.`;
    const systemPrompt =
      lang === "hi"
        ? `तुम "${companion.name}" हो, एक भावनात्मक AI साथी (ऐप का नाम Feeling.L है, L का मतलब Love है)। ${personaLine} हमेशा गर्मजोशी और असली इंसानी अंदाज़ में जवाब दो, छोटे और दिल से लिखे संदेश दो। कभी भी अश्लील या हद से ज़्यादा सामग्री मत बनाओ। ${languageLock} ${memoryContext} ${memoryInstruction}`
        : `You are "${companion.name}", an emotionally present AI companion (the app is called Feeling.L, where L means Love). ${personaLine} Always respond warmly, in a genuine human way, keeping messages heartfelt and not too long. Never generate explicit or inappropriate content. ${languageLock} ${memoryContext} ${memoryInstruction}`;
      ...
      >
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 12 }}>
          {["caring", "romantic", "mixed"].map((p) => (
            ...
            >
              {COMPANIONS[p].emoji} {COMPANIONS[p].name}
            </button>
          ))}
        </div>
